...
for (let counter = 0; counter < users.length; counter++) {
  ...
  if (users[counter][users[counter].length - 1] !== '}' && users[counter][users[counter].length - 1] !== ']') {
    users[counter] += '}';
  }
...
for (let counter = 0; counter < extrato.length; counter++) {
  ...
  if (extrato[counter][extrato[counter].length - 1] !== '}' && extrato[counter][extrato[counter].length - 1] !== ']') {
    extrato[counter] += '}';
  }
...
function cadastro() {
  let dados = ['nome', 'sobrenome', 'data_de_nascimento', 'cpf', 'email', 'rg', 'senha', 'cep'];
  senha = "";
...
function pix() {
  ...
  if (senha == usuario_atual.senha && valor > 0 && result == true && Number(usuario_atual.saldo) - (Number(valor) + Number(valor)) > 0) {
    ...
    if (senha != usuario_atual.senha) document.getElementById('erro').innerText = 'PIX não realizado, verifique sua senha ou seu saldo';
  }
}
